...
export async function signUp(req, res) {
    ...
    try {
        ...
        const token = jwt.sign({ email: newUser.email, id: newUser._id }, "thisis", { expiresIn: '1d' });

        res.status(201).json({ newUser, token });
    } catch (error) {
        console.error(error);
        res.status(510).json({ message: 'Server error' });
    }
}
...
export async function signIn(req, res) {
    ...
    try {
        ...
        // Generate a JWT token
        const token = jwt.sign({ email: existingUser.email, id: existingUser._id }, "thisis", { expiresIn: '1d' });
